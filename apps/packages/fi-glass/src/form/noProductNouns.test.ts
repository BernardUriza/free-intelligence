/**
 * fi-glass must not learn the consumer's noun — the form layer.
 *
...
describe('the form primitives stay free of the consumer vocabulary', () => {
  ...
  it('scans the module (a rename must not silently empty this guard)', () => {
    expect(sources).toEqual(
      expect.arrayContaining(['Field.tsx', 'Button.tsx', 'controls.tsx', 'formStyle.ts']),
    );
  });
