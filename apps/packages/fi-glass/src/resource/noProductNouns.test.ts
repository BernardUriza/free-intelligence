/**
 * fi-glass must not learn the consumer's noun (FIGLASS-PROJECTS-PAGE-1).
 *
...
describe('the resource primitives stay free of the consumer vocabulary', () => {
  const sources = readdirSync(DIR).filter(
    (f) => (f.endsWith('.ts') || f.endsWith('.tsx')) && !f.includes('.test.'),
  );

  it('has sources to check (a rename must not silently empty this guard)', () => {
