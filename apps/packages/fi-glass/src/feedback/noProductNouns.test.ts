/**
 * fi-glass must not learn the consumer's noun — the feedback layer.
 *
...
describe('the feedback primitives stay free of the consumer vocabulary', () => {
  ...
  it('scans the module (a rename must not silently empty this guard)', () => {
    expect(sources).toEqual(
      expect.arrayContaining(['Callout.tsx', 'feedbackStyle.ts']),
    );
  });
