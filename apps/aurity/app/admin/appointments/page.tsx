...
export default function AppointmentsCalendarPage() {
  ...
  // Modal close handlers (single source of truth)
  ...
  const handleSubmitNewAppointment = useCallback(async (data: Record<string, unknown>) => {
    await handleCreateAppointment({
      ...data,
      scheduled_at: typeof data.scheduled_at === 'string'
        ? data.scheduled_at
        : (data.scheduled_at as Date).toISOString(),
      clinic_id: selectedClinic,
      status: 'scheduled',
      checkin_code: '',
    } as Omit<Appointment, 'appointment_id'>);
    setPrefilledData(null);
  }, [selectedClinic, handleCreateAppointment]);

  const handleSubmitEditAppointment = useCallback(async (payload: Record<string, unknown>) => {
    const aptId = (payload.appointment_id as string) || selectedAppointment?.appointment_id || '';
    if (!aptId) return;
  ...
  }, [selectedAppointment, handleEventEdit, closeEditAppointmentModal]);
  ...
  return (
    <>
      ...
      <EditAppointmentModal
        ...
        onCancel={closeEditAppointmentModal}
