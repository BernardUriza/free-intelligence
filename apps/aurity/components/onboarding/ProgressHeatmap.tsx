...
import { Check, CheckCircle, Timer } from 'lucide-react';
            ...
            <div key={phase.name} className="group relative">
              ...
              >
                {phase.completed && (
                  <div className="w-full h-full flex items-center justify-center text-emerald-200">
                    <Check className="w-6 h-6" strokeWidth={2} aria-hidden="true" />
                  </div>
                )}
              </div>
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                <div className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 shadow-xl whitespace-nowrap">
                  ...
                  <p className="fi-text-xs mt-1 flex items-center gap-1">
                    {phase.completed ? <><CheckCircle className="w-3 h-3 text-emerald-400" strokeWidth={1.5} aria-hidden="true" /> Completed</> : <><Timer className="w-3 h-3 text-yellow-400" strokeWidth={1.5} aria-hidden="true" /> Pending</>}
                  </p>
                  {phase.completed && (
