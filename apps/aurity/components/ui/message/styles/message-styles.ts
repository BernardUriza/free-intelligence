 ...
 * Shared styles for all message variants
 *
...
// TV Display styles (large text, responsive)
export const tvStyles = {
  message: {
    base: 'flex-1 flex flex-col min-h-0 bg-gradient-to-br from-purple-950/40 to-slate-950/40 border border-purple-600/30 rounded-xl p-4 sm:p-6 lg:p-8',
  },
  content: {
    base: 'flex-1 flex items-center justify-center min-h-0 w-full',
    text: 'text-white text-center leading-snug font-semibold w-full px-4',
  },
  header: {
    container: 'flex items-center gap-3 mb-4 flex-shrink-0',
    badge: 'bg-purple-500/20 rounded-full px-3 py-1.5 sm:px-4 sm:py-2',
    label: 'font-semibold text-purple-300 tracking-wide',
  },
} as const;

/**
 * Calculate TV font size based on content length
 */
export function getTVFontSize(contentLength: number): string {
