...
| [fi-core: `GravityScore.reasons` estructurados — el veredicto explica con nombres de grupo, nunca con la frase](fi-core-urgency-structured-reasons.md) | **Done 2026-09-08** — fi-core 0.30.0 (PR #465): `UrgencyReason(kind, key, weight, term)`, `term` fuera del `repr`, `explain()` conserva la prosa. Falta el recibo del consumidor: discord-bot loguea `key`/`weight` | 2026-09-08 |
| [fi-core: el veredicto de banda carga sus grupos disparados — `ClinicalDomain.assess()` → `ClinicalVerdict`](fi-core-verdict-carries-matched-groups.md) | **Done 2026-09-08** — fi-core 0.30.0 (PR #465): una lectura, un veredicto; `_GROUP_TO_CONDITION` ya vivía en fi-core como `SignalGroup.category`. Falta que discord-bot colapse a `assess()` | 2026-09-08 |
| [fi-core: primitivas de auditoría — seudónimo HMAC con llave que rota por mes + evento con `audit_hash`](fi-core-audit-primitives.md) | **Done 2026-09-08** — fi-core 0.30.0 (PR #465): `fi_core.audit` (`pseudonym` / `audit_period` / `audited`), byte-compatible con el canary. Falta que discord-bot `audit.py` apunte ahí | 2026-09-08 |
