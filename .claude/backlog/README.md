...
| Item | Status | Propuesta |
|---|---|---|
| [OG118-BACKGROUND-1 — ejecución real en background (que "te aviso" sea verdad)](og118-real-background-execution.md) | **In progress** (2026-09-12) — decidido por `/ultra-lord`: ACA Job. La tarjeta decía que rompía el invariante de cero estado; verificado que las conversaciones ya viven en Azure Files y que la restricción real era `minReplicas: 0`. Construido: tool `start_background_task` por MCP remoto, worker en Job, `append_message` + anti-clobber, poll del cliente. Falta el recibo E2E tras el deploy | 2026-07-05 |
...
| [RESONANCE — modo llamada de voz sin pantalla](og118-resonance-voice-mode.md) | **Done — default-on 2026-09-12** — máquina de llamada, barge-in, VAD, hangup por inactividad y `maxCallMs` en fi-glass, consumidos por og118; fallos de voz al banner; cuota por principal en el servidor. El stress-test del 09-09 salió rojo por el techo de 3 RPM; las solicitudes a Azure se concedieron el 12-09 (`whisper`/`tts` 3 → 30 RPM, deployments escalados, usage 30.0/30.0), y ese mismo día se borró el flag `?resonance=1` y el default por principal subió a 10 RPM | 2026-06-29 |
