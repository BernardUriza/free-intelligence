...
| [fi-core: `GravityScore.reasons` estructurados — el veredicto explica con nombres de grupo, nunca con la frase](fi-core-urgency-structured-reasons.md) | Proposed — `reasons` de fi-core trae la frase emparejada y discord-bot #54 la logueó tal cual en Log Analytics (persona-gateway rev 201); `UrgencyReason(kind, key, weight)` + `render()`, MINOR con nota de migración | 2026-09-08 |
| [fi-core: el veredicto de banda carga sus grupos disparados — `ClinicalDomain.assess()` → `ClinicalVerdict`](fi-core-verdict-carries-matched-groups.md) | Proposed — hoy banda y explicación salen de dos motores que ya divergieron una vez (0.29.1); una llamada que devuelva score + acute + chronic, y `_GROUP_TO_CONDITION` sube del consumidor al dominio. Último tramo de [[fi-core-weighted-clinical-domain]] | 2026-09-08 |
| [fi-core: primitivas de auditoría — seudónimo HMAC con llave que rota por mes + evento con `audit_hash`](fi-core-audit-primitives.md) | Proposed — discord-bot #68 escribió `pseudonymous_user` y `_emit` en `khimeras_shared/audit.py`; og118/AIRE las van a necesitar igual. Suben como `pseudonym` / `audit_period` / `audited` sin structlog; tests portados, el del sha256 pelón obligatorio | 2026-09-08 |
