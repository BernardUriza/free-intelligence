// Capture all Fi-Monitor tabs with implemented features
import fs from 'fs';
...
async function main() {
  ...
  // Tabs to capture: Services (0), Config (2), Benchmarks (4)
  const tabsToCapture = [
    { index: 0, name: 'services', description: 'Services tab with Model Manager' },
  ...
  for (const tab of tabsToCapture) {
    ...
    // For Services tab, also click "Manage Models" button if Ollama is on
    if (tab.index === 0) {
      // Click manage models button
      await sendCommand('Runtime.evaluate', {
        expression: `
          const manageBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Manage Models'));
  ...
  ws.close();
  console.log('\n✅ All 3 feature screenshots captured!');
}
