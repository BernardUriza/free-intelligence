...
from ..infrastructure.middleware import get_current_user, require_permission, require_roles
...
async def rbac_matrix() -> dict:
    matrix = {
        role.value: sorted(list(perms)) for role, perms in DEFAULT_ROLE_PERMISSIONS.items()
    }
    return {"roles": matrix}
