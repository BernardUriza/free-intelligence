...
Incremental audio concatenation on each PAUSE.

Python 2026 patterns:
- Annotated[T, Depends(...)] for cleaner DI
- dataclass-style Pydantic models with Field descriptions
...
From: backend/api/routers/session/internal/sessions/checkpoint.py
"""
...
async def checkpoint_session(
    ...
    try:
        ...
        logger.error("CHECKPOINT_TOO_MANY_CHUNKS", session_id=session_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
        ) from e
        ...
        logger.error("CHECKPOINT_FAILED", session_id=session_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Checkpoint failed: {e!s}",
        ) from e
    except Exception as e:
        logger.error("CHECKPOINT_UNEXPECTED_ERROR", session_id=session_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Checkpoint failed: {e!s}",
        ) from e
