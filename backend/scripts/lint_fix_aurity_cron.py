...
"""Continuous AURITY lint fix cron job - Runs lint_fix_aurity_worker in a loop."""
...
def main():
    """Run continuous AURITY lint fixing in batches of 5."""
    logger.info("lint_fix_aurity_cron_started")
    ...
    while True:
        try:
            logger.info("starting_lint_fix_aurity_batch", batch_size=batch_size)
            result = lint_fix_aurity_worker(batch_size)
            data = result

            logger.info(
                "lint_fix_aurity_batch_result",
                status=data["status"],
            ...
            logger.info("lint_fix_aurity_cron_stopped_by_user")
            break
        except Exception as e:
            logger.error("lint_fix_aurity_cron_error", error=str(e))
