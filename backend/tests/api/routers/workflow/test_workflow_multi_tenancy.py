...
@pytest.mark.xfail(reason="Document search does not filter by clinic_id - SECURITY BUG")
def test_document_search_filters_by_clinic(client, doctor_a_clinic1):
    ...
    ⚠️ EXPECTED TO FAIL - document search returns results from all clinics
    This allows semantic search to leak PHI across clinic boundaries.
    """
    public_app.dependency_overrides[get_current_user] = lambda: doctor_a_clinic1
    ...
    # Should NOT be 403 (user can search their own clinic's documents)
    assert response.status_code == status.HTTP_200_OK
