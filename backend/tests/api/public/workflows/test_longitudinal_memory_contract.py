...
def test_memory_route_registration_check() -> None:
    ...
    assert "/workflows/aurity/timeline/memory/stats" in routes, \
        "Stats endpoint not registered"
    
    assert "/workflows/aurity/timeline/memory/search" in routes, \
        "Search endpoint not registered"


def test_memory_response_schema_structure() -> None:
    """Verify LongitudinalMemoryResponse has correct structure."""
    from backend.api.public.workflows.longitudinal_memory import LongitudinalMemoryResponse
...
def test_memory_event_schema_structure() -> None:
    ...
    from backend.api.public.workflows.longitudinal_memory import MemoryEvent
...
def test_memory_stats_response_schema_structure() -> None:
    ...
    from backend.api.public.workflows.longitudinal_memory import MemoryStatsResponse
...
def test_stable_routes_not_deprecated() -> None:
    ...
    routes_with_status = [
        (r.path, getattr(r, 'deprecated', False))
    ...
    stable_routes = [
        (path, deprecated)
        for path, deprecated in routes_with_status
        if '/timeline/memory' in path
    ]
    ...
    for path, deprecated in stable_routes:
        assert deprecated is not True, \
            f"Stable route {path} incorrectly marked as deprecated"
