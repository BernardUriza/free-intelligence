...
class TestDIContainer:
    ...
    def test_container_singleton_pattern(self, di_container):
        ...
        service1 = di_container.get_audit_service()
        service2 = di_container.get_audit_service()
...
class TestContainerErrorHandling:
    ...
    def test_container_service_dependency_chain(self, di_container):
        ...
        # AuditService depends on AuditRepository
        audit_service = di_container.get_audit_service()

        # Verify repository is injected
        assert hasattr(audit_service, "repository")
        assert audit_service.repository is not None

        # Repository should be the same instance from container
        repo_from_container = di_container.get_audit_repository()
        assert audit_service.repository is repo_from_container
