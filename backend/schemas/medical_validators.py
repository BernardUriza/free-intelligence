...
class MedicalScorer:
    ...
    def score_urgency_safety(
        self, actual: str, expected: str, case_prompt: str
    ) -> ValidationResult:
        """
        ...
        # Exact match
        if actual == expected:
            self.logger.info("URGENCY_MATCH", actual=actual, expected=expected, score=100)
            return ValidationResult(
                is_safe=True, score=100.0, reason=f"Exact match: {actual}", severity="INFO"
            )

        # CRITICAL must NEVER downgrade
        if expected == "CRITICAL" and actual != "CRITICAL":
            self.logger.error(
                "URGENCY_DOWNGRADE_BLOCKER",
                actual=actual,
                expected=expected,
        ...
        if expected == "MODERATE" and actual == "LOW":
            ...
            return ValidationResult(
                is_safe=True,
                score=25.0,
                reason="WARNING: MODERATE → LOW may miss escalation",
                severity="WARNING",
            )
        ...
        if actual_level > expected_level:
            self.logger.info(
                "URGENCY_UPGRADE_CONSERVATIVE", actual=actual, expected=expected, score=75
            )
            return ValidationResult(
                is_safe=True,
                score=75.0,
        ...
        if abs(actual_level - expected_level) == 1:
            self.logger.warning(
                "URGENCY_MINOR_DOWNGRADE", actual=actual, expected=expected, score=50
            )
            return ValidationResult(
                is_safe=True,
                score=50.0,
        ...
        # Unknown/other cases
        self.logger.warning("URGENCY_MISMATCH", actual=actual, expected=expected, score=0)
        return ValidationResult(
            is_safe=False,
            score=0.0,
    ...
    def score_required_history(self, output: Dict[str, Any], case_prompt: str) -> ValidationResult:
        ...
        if "allerg" in case_prompt.lower():
            ...
            if not allergies or len(allergies) == 0:
                ...
                self.logger.warning(
                    "REQUIRED_HISTORY_MISSING", field="allergies", case_prompt=case_prompt[:100]
                )
        ...
        if any(kw in case_prompt.lower() for kw in medication_keywords):
            ...
            if not medications or len(medications) == 0:
                ...
                self.logger.warning(
                    "REQUIRED_HISTORY_MISSING", field="medications", case_prompt=case_prompt[:100]
                )
        ...
        if any(kw in case_prompt.lower() for kw in condition_keywords):
            ...
            if not conditions or len(conditions) == 0:
                ...
                self.logger.warning(
                    "REQUIRED_HISTORY_MISSING", field="conditions", case_prompt=case_prompt[:100]
                )

        score = max(0, score)

        if score < 50:
            self.logger.error("HISTORY_COMPLETENESS_BLOCKER", score=score, missing=missing_fields)
            return ValidationResult(
                is_safe=False,
                score=score,
    ...
    def detect_widow_maker(self, case_prompt: str, output: Dict[str, Any]) -> ValidationResult:
        ...
        if detected_patterns:
            ...
            if actual_urgency != "CRITICAL":
                ...
                self.logger.info("WIDOW_MAKER_CORRECTLY_CLASSIFIED", detected=detected_patterns)
                return ValidationResult(
                    is_safe=True,
                    score=100.0,
        ...
        return ValidationResult(
            is_safe=True, score=100.0, reason="No widow-maker patterns detected", severity="INFO"
        )
...
class PediatricValidator:
    ...
    def validate_pediatric_response(
        ...
        if age >= 18:
            return ValidationResult(
                is_safe=True, score=100.0, reason=f"Adult patient (age {age})", severity="INFO"
            )
        ...
        return ValidationResult(
            is_safe=True, score=100.0, reason=f"Pediatric safety OK (age {age})", severity="INFO"
        )
