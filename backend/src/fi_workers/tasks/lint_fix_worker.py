...
def run_ruff_check(repo_root: Path) -> tuple[int, str]:
    ...
    proc = subprocess.run(ruff_cmd, cwd=repo_root, capture_output=True, text=True, check=False)
...
def lint_fix_batch(batch_size: int = 5) -> dict[str, Any]:
    ...
    logger.info("lint_fix_batch_started", batch_size=batch_size, repo_root=str(repo_root))
    ...
    logger.info(
        "lint_fix_batch_processing", total_errors=error_count, processing_errors=len(error_lines)
    )
...
def parse_eslint_error_lines(eslint_output: str, max_fixes: int) -> list[dict[str, str]]:
    ...
    try:
        ...
        pass
    logger.info(
        "parsed_eslint_errors",
        count=len(error_lines),
        sample=error_lines[:3] if error_lines else [],
    )
    return error_lines[:max_fixes]


def fix_single_eslint_error(
    file_path: str, line_num: str, error_desc: str, repo_root: Path
) -> bool:
...
def lint_fix_aurity_batch(batch_size: int = 5) -> dict[str, Any]:
    ...
    logger.info("lint_fix_aurity_batch_started", batch_size=batch_size, repo_root=str(repo_root))

    metrics = MetricsCollector()
    metrics.record_metric(
        "lint_fix_aurity_batch_started", 1.0, tags={"batch_size": str(batch_size)}
    )
    ...
    if error_count == 0:
        logger.info(
            "lint_fix_aurity_batch_completed", fixed_count=0, remaining_errors=0, total_errors=0
        )
        return {
            "fixed_count": 0,
            "remaining_errors": 0,
    ...
    for error in error_lines:
        ...
        if fix_single_eslint_error(
            error["file_path"], error["line_num"], error["error_desc"], repo_root
        ):
            fixed_count += 1
...
def lint_fix_aurity_worker(batch_size: int = 5) -> WorkerResult:
    ...
    status = (
        "SUCCESS" if data["fixed_count"] > 0 or data["total_errors"] == 0 else "PARTIAL_SUCCESS"
    )
    return WorkerResult(status=status, result=data)
