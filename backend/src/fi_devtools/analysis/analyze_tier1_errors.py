...
Extracts reportUndefinedVariable and reportArgumentType errors
from pyright results for prioritized fixing.

Usage:
    fi analysis analyze-tier1-errors
    fi analysis analyze-tier1-errors ops/type_check_results/results.json
"""
...
def analyze_tier1_errors(results_path: Path) -> None:
    ...
    output_path = results_path.parent / "tier1_errors.json"
    ...
    for rule in tier1_rules:
        ...
        if errors:
            ...
            for error in errors[:5]:
                ...
                if "/backend/" in file_path:
                    file_path = file_path.split("/backend/")[-1]
                line = error.get("line", "?")
