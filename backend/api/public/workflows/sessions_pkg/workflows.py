...
async def diarize_session_workflow(session_id: str) -> dict:
    ...
    try:
        ...
        logger.error("DIARIZATION_WORKFLOW_FAILED", session_id=session_id, error=str(e), exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to dispatch diarization: {e!s}") from e
...
async def generate_soap_workflow(session_id: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to dispatch SOAP generation: {e!s}") from e
...
async def analyze_emotion_workflow(session_id: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to dispatch emotion analysis: {e!s}") from e
...
async def analyze_session_intelligent_workflow(
    ...
    from backend.storage.task_repository import CORPUS_PATH, ensure_task_exists, get_task_metadata
    from backend.workers.executor_pool import spawn_worker

    try:
        logger.info("INTELLIGENT_WORKFLOW_STARTED", session_id=session_id, audio_duration=audio_duration_seconds, language=language)
            ...
            try:
                with h5py.File(CORPUS_PATH, "r") as f:
                    ...
                    if audio_path in f:
                        ...
                        logger.info("AUDIO_DURATION_DETECTED", session_id=session_id, duration_seconds=audio_duration_seconds)
            except Exception as e:
                logger.warning("AUDIO_DURATION_DETECTION_FAILED", session_id=session_id, error=str(e), hint="Using default duration of 60s")
        ...
        logger.info("WORKFLOW_ROUTING_DECISION", session_id=session_id, workflows=decision["workflows"], reasoning=decision["reasoning"], cost_usd=decision["cost"].routing_cost_usd, savings_usd=decision["cost"].execution_cost_saved_usd)
        ...
        for workflow in decision["workflows"]:
            ...
            if task_type == TaskType.TRANSCRIPTION:
                logger.info("TRANSCRIPTION_SKIPPED", session_id=session_id, hint="Transcription handled during upload")
                continue
        ...
        return {
            ...
            "cost": {
                ...
                "net_savings_usd": (decision["cost"].execution_cost_saved_usd - decision["cost"].routing_cost_usd),
            },
        ...
        logger.error("INTELLIGENT_WORKFLOW_FAILED", session_id=session_id, error=str(e), exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to orchestrate workflows: {e!s}") from e


@router.post("/sessions/{session_id}/finalize", status_code=status.HTTP_202_ACCEPTED)
async def finalize_session_workflow(session_id: str, request: FinalizeSessionRequest) -> FinalizeSessionResponse:
    from backend.api.internal.sessions.finalize import finalize_session as internal_finalize
    try:
        logger.info("FINALIZE_SESSION_WORKFLOW_STARTED", session_id=session_id, sources_count=len(request.transcription_sources.webspeech_final))
        ...
        logger.error("FINALIZE_SESSION_WORKFLOW_FAILED", session_id=session_id, error=str(e), exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to finalize session: {e!s}") from e


@router.post("/sessions/{session_id}/checkpoint", status_code=status.HTTP_200_OK)
async def checkpoint_session_workflow(session_id: str, request: CheckpointRequest) -> CheckpointResponse:
    from backend.api.internal.sessions.checkpoint import checkpoint_session as internal_checkpoint
    try:
        logger.info("CHECKPOINT_WORKFLOW_STARTED", session_id=session_id, last_chunk_idx=request.last_chunk_idx)
        result = await internal_checkpoint(session_id, request)
        logger.info("CHECKPOINT_WORKFLOW_SUCCESS", session_id=session_id, chunks_concatenated=(result.chunks_concatenated if hasattr(result, "chunks_concatenated") else result.get("chunks_concatenated")))
        if isinstance(result, dict):
            result_dict: dict[str, Any] = result  # type: ignore[name-defined]
        else:
            result_dict = result.model_dump()
        ...
        logger.error("CHECKPOINT_WORKFLOW_FAILED", session_id=session_id, error=str(e), exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to checkpoint session: {e!s}") from e
