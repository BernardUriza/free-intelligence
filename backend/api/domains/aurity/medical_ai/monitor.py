"""Session progress monitoring endpoint.

Provides real-time progress tracking for all medical AI tasks:
- Transcription progress (chunks processed)
- Diarization progress (speaker separation)
- SOAP generation status
- Encryption status

Migrated from: backend/api/routers/session/public/sessions_pkg/monitor.py
"""
...
async def monitor_session_progress(
    ...
    try:
        ...
        try:
            diarization_meta = task_repo.get_task_metadata(session_id, TaskType.DIARIZATION) or {}
        ...
        try:
            soap_meta = task_repo.get_task_metadata(session_id, TaskType.SOAP_GENERATION) or {}
        ...
        try:
            encryption_meta = task_repo.get_task_metadata(session_id, TaskType.ENCRYPTION) or {}
