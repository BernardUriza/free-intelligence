...
class TestSprintAutomation(unittest.TestCase):
    ...
    def test_dry_run_no_side_effects(self) -> None:
        ...
        # Get current bundles
        before_bundles = (
            list(Path(self.backup_dir).glob("*.bundle")) if os.path.exists(self.backup_dir) else []
        )
        ...
        # Verify no new tags
        after_tags = subprocess.check_output(["git", "tag", "-l"], text=True).strip().split("\n")

        self.assertEqual(set(before_tags), set(after_tags), "DRY_RUN should not create tags")
    ...
    def test_release_notes_generation(self) -> None:
        ...
        self.assertTrue(
            os.path.exists(notes_path), f"Release notes file should exist: {notes_path}"
        )
    ...
    def test_no_commits_scenario(self) -> None:
        ...
        self.assertTrue("Commits desde tag:" in result.stdout or "Sin cambios" in result.stdout)
...
class TestSprintWorkflow(unittest.TestCase):
    ...
    def test_backups_directory_gitignored(self) -> None:
        ...
        if os.path.exists(gitignore_path):
            ...
            self.assertIn("backups/", gitignore, "backups/ should be gitignored")
